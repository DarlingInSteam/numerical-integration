...
def simpson_with_runge(left, right, function, epsilon):
    n = 2
    h = (right - left) / (2 * n)
    ...
    while abs(tmp_sum - prev_sum) > epsilon:
        ...
        # Используем правило Рунге для контроля точности
        runge_sum = float(function(left)) + \
                    float(function(right))

        for step in range(1, 2 * n, 2):
            runge_sum += 4 * float(function(left + step * h))

        for step in range(2, 2 * n - 1, 2):
            runge_sum += 2 * float(function(left + step * h))

        runge_sum *= h / 3

        error = abs(runge_sum - tmp_sum) / 15
        if error > epsilon:
            n *= 2
            h /= 2

    print(tmp_sum, step+2)
...
simpson_with_runge(5, 10, f, 0.00001)
